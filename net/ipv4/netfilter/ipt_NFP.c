...
extern int nfp_ct_learn_en;

#ifdef CONFIG_MV_ETH_NFP_NAT
extern int move_nat_to_nfp(const struct nf_conntrack_tuple *tuple,
...
void nfp_print_info(struct ipt_nfp_info *info)
{
	...
	printk(KERN_INFO "info = %p: mode = %d, flags = 0x%X, burst = %d, limit = %d\n",
		info, info->mode, info->flags, info->burst, info->limit);
	...
	printk(KERN_INFO "txq = %d, txp = %d, mh = 0x%X, entry = %p\n", info->txq, info->txp, info->mh, info->entry);
}
